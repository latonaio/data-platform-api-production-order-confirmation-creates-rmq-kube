...
import (
	"context"
	dpfm_api_caller "data-platform-api-production-order-confirmation-creates-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-production-order-confirmation-creates-rmq-kube/existence_conf"
	"data-platform-api-production-order-confirmation-creates-rmq-kube/sub_func_complementer"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncProductionOrderConfirmationCreates(accepter, &input, &output, l)
	if len(errs) != 0 {
